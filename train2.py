...
#         # Loss function
#         class_counts = [0, 0]
#         for item in train_dataset.samples[book_name]:
#             class_counts[item['label']] += 1
#         total_samples = sum(class_counts)
...
#             for item in train_dataset.samples[book_name]:
#                 text = "Caption: " + item['caption'] + " Character: " + item['char'] + " Content: " + item['content']
#                 tokens = BackstoryDataset.text_to_tokens(train_dataset, text)
#                 chunks, mask = BackstoryDataset.chunk_tokens(train_dataset, tokens)
#                 chunks = chunks.to(device)
#                 mask = mask.to(device)
#                 label = torch.tensor([item['label']], device=device)
...
#                 with ctx:
#                     # Process all chunks for this sample
#                     # Mean pool across chunks for long documents
#                     all_logits = []
#                     for chunk_idx in range(chunks.size(0)):
#                         chunk = chunks[chunk_idx:chunk_idx+1]  # (1, block_size)
#                         chunk_mask = mask[chunk_idx:chunk_idx+1]
#                         logits = model(chunk, chunk_mask)  # (1, num_classes)
#                         all_logits.append(logits)
                    
#                     # Average logits across all chunks (hierarchical pooling)
...
#                     print(f"Epoch {epoch+1}/{MAX_EPOCHS} | Step {step}/{len(train_dataset)} | "
#                         f"Loss: {epoch_loss/total:.4f} | Acc: {acc:.1f}%")
            
#             epoch_acc = correct / total * 100
#             print(f"\n[Epoch {epoch+1}] Loss: {epoch_loss/total:.4f} | Accuracy: {epoch_acc:.1f}%\n")
        
#         torch.save(model.state_dict(), f'{book_name.lower()}.pt')    

def evaluate_classifier():
    test_dataset = BackstoryDataset("eval.csv")
    for book_name in test_dataset.samples.keys():
        model = BackstoryClassifier(BDH_CONFIG, num_classes=2).to(device)
        model_path = os.path.join(os.path.dirname(__file__), f"{book_name.lower()}.pt")
        model.load_state_dict(torch.load(model_path, map_location=device, weights_only=True), strict=False)
        ...
        with torch.no_grad():
            for item in test_dataset.samples[book_name]:
                text = "Caption: " + item['caption'] + " Character: " + item['char'] + " Content: " + item['content']
                tokens = BackstoryDataset.text_to_tokens(test_dataset, text)
                chunks, mask = BackstoryDataset.chunk_tokens(test_dataset, tokens)
                chunks = chunks.to(device)
                mask = mask.to(device)
                label = item['label']
